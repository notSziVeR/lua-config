quest Marriage_Manage begin
	state start begin
		when oldwoman.chat."WeŸ Œlub" with not pc.is_engaged_or_married() begin
			...
			if not m_has_ring then
				text_center() say_title(string.format("%s:", mob_name(npc.get_race())))
				say("_____________________________________________________")
				say("Musisz zdobyæ Pierœcionek Zarêczynowy.")
				say("Dopiero wtedy bêdziesz móg³ wzi¹æ œlub.")
				say("")
			...
			if not Marriage_Manage.is_equip_wedding_dress() then
				text_center() say_title(string.format("%s:", mob_name(npc.get_race())))
				say("_____________________________________________________")
				say("Chcesz braæ œlub w tych szmatach?")
				say("Nie rób sobie wstydu i ubierz siê jak")
